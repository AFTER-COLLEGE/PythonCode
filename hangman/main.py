...
def print_current__state(current_word_state,attempts_remaining):
    #it will print current status of the game that is word and attempt left
...
def check_game_status(selected_word,current_word_state,attempts_remaining):
    if attempts_remaining <=0:
        print("SOORY YOU LOST :(   TRY AGAIN")
...
def play_game(attempt=5):
    #if contain main logic of program

    #it will store random word
    ...
    print_current__state(current_word_state,attempts_remaining)
    
    while True:
        input_char=input("Guess The Charecter: ")
        print("")
